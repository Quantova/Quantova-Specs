...
A committee of about 500 is sampled per round from the active set by the verifiable random function seeded from the previous block beacon. A bond is locked for twelve months before it can be unbonded at all, which is a deliberate departure that selects committed capital, and after that lock an exit request enters a fourteen day unbonding period during which the stake stays slashable. Exiting is a right and is never itself slashed. The lock, the unbonding, and the check of that period against the time evidence takes to surface are set out in the economics specification and in OPTIONS-validator-economics.md.
