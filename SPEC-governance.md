...
QONCORD is the governance protocol of Quantova. It runs parallel referendum tracks with conviction voting and delegation, and it rebuilds all of that on post quantum foundations. Every ballot is a lattice signature. Every tally is a STARK certificate that travels the same wrapper as consensus. Governance is bounded by a constitution that no track can cross.
...
1.3 Bridge pool migration. Moves the bridge pool to a new vault, a high value custody action. Its deposit is 200 thousand QTOV. It passes at 80 percent approval with 40 percent support and runs five days. In an emergency the bridge is frozen first and the pool migrates under this vote inside the freeze window.

1.4 Freeze and asset recovery. Emergency consumer protection. Its deposit is 150 thousand QTOV. It passes at 75 percent approval with 30 percent support and ratifies in about six hours. The freeze itself is instant and handled by the guardian caucus in section 4. This vote ratifies the clawback fast enough to catch a thief. The full amount returns to the address it was stolen from, and ordinary users are never affected.

1.5 Blacklist and kill address. Retires a compromised or hostile address. Its deposit is 200 thousand QTOV. It passes at 75 percent approval with 30 percent support and runs about two days.

1.6 Add asset to bridge. Lists a new project's asset on the bridge. Its deposit is 30 thousand QTOV. It passes at 60 percent approval with 10 percent support and runs seven days. The refundable deposit plus a public approval keeps fraudulent or low quality tokens off the bridge.

1.7 Parameter change, a QIP. Ordinary tuning of a chain parameter, the everyday track. Its deposit is 15 thousand QTOV. It passes at 50 percent approval with 4 percent support and runs seven days.

## 2. Voting

A voter's base weight is their staked QTOV. A voter may strengthen a vote by locking stake for conviction, and the multiplier runs up to 2.5 times, where weight is stake times the conviction factor. Locked QTOV returns automatically to the same address when the lock ends. Delegation is chosen per track, so a holder can delegate one track and vote directly on another.
