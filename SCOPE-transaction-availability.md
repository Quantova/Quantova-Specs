...
At the working design target and a measured per transaction signature size, a node that downloads every transaction faces sustained bandwidth well above the one gigabit floor described below, no rate or bandwidth figure stated here since throughput is withheld everywhere until the finished stack is benchmarked once. Erasure coding does not change that for a node that must reconstruct the block, by information theory reconstruction gathers a whole block. So the floor is lowered only two ways, a smaller transaction, or fewer nodes needing the whole transaction stream. This paper scopes the second.
