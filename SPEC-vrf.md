...
The per user function lets a caller derive a random output bound to an input. The output is SHAKE256 over the ML DSA signature of the input, followed by the input. The proof is the signature together with a STARK that the derivation is correct. Uniqueness comes from deterministic ML DSA signing, since the same signer and the same input yield one signature. Verifiability comes from rechecking the signature and the proof. This function maps to the virtual machine opcode that verifies a random output.
