...
Status. This is a plan for the founder, not normative and not an implementation. It sets out the run that produces the first real network throughput and finality figure, the hosts it needs, what it measures and isolates, and the prerequisites that must be resolved first. It builds nothing. A parallelism variable is already committed in a loopback results file elsewhere, outside this documentation set, so this run adds one variable on top of a component result that already exists, without restating that component result here.

## What it measures, and the one variable it adds

The loopback run isolated parallelism, real parallel per process compute over localhost sockets with near zero propagation, and its committed result lives in its own results file rather than in this document. This run adds propagation, real geographic network latency between validators, on top of that parallelism. Because parallelism is already measured elsewhere, the difference between this run and the loopback run is the propagation cost, so the two runs together separate the two variables that a single distributed run would have bundled. This run produces the first real network sustained finalised throughput and the first real finality distribution with a tail that includes the wire, and like every other performance figure it is published only as its own committed results file once it exists, not as prose here.
