...
An account may name a set of guardians, each an account with its own post quantum key. A recovery is a request signed by the account owner where possible, or proposed by the guardians, and it takes effect only when a threshold of the guardians approve it with their module lattice signatures. The threshold and the guardian set are chosen by the account owner.
