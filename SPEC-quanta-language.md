...
The unsigned types are u8, u16, u32, u64, and u128. There are no signed types at the surface. The machine computes in unsigned 64 bit words and has no signed operation, so a signed type would be a promise the machine cannot enforce, and the language does not advertise what the machine cannot hold. Signed types return when the machine has native signed operations and a 128 bit word to carry i128, and not before. There is no uint256 and no 256 bit surface integer, for the same reason. Asset amounts are u128 inside the asset primitive, so a developer never picks a width for money. Integer width has no bearing on quantum security, since the keys and hashes and signatures are all 256 bit class NIST primitives.
